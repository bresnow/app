const JOY = window.JOY = function () { };
const as = window.as = function (element, gun, cb, opt) {
	element = $(element);
	if (gun === as._gun && as.element && as.element.is(element)) {
		log("As in use")
		return
	}
	...
	function template(tag, attr) {
		...
		var data = [], plate = attr ? tag.attr(attr) : tag.innerHTML;
	...
	as.element = element;
	if (element.data('as')) {
		element.html(element.data('as').fresh);
	} else {
		element.data('as', {
			fresh: element.html()
		})
	...
	element.find("[name]").each(function () {
		...
		ref.get(function (at) {
			var data = at.put, key = at.get, _gun = at.gun || at.$, ui = name, back;
			if (model) {
				ui = model.has[(_gun._).id];
				if (!ui) {
					back = _gun.back(many - 1);
...
as.sort = function(num, li) { return parseFloat(num) >= parseFloat($(li).find('.sort').text() || -Infinity) ? li : sort(num, li.prev()) }
...
; (function () {
	...
	r.page = function (hash, cb) {
		r.page[hash] = r.page[hash] || { on: cb };
		return r;
	}
